...
/** Official Hack the North gear/cog logo mark (light variant for dark backgrounds).
 *  Source: https://github.com/hackthenorth/2020.hackthenorth.com — src/static/img/logos/icon-light.svg */
function HtnLogo({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 0 61 58"
...
/** Dark cinematic navbar — nearly invisible against the void. */
export function Navbar() {
  const { isAuthenticated, logout } = useAuth();

  return (
    <nav
      className="sticky top-0 z-50 border-b border-[#111827]/50 bg-[#05060f]/80 backdrop-blur-xl"
      aria-label="Main navigation"
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link
          href="/"
          className="flex items-center gap-2.5 rounded-lg px-2 py-1 text-lg font-bold uppercase tracking-[0.1em] text-slate-200 transition-colors hover:text-violet-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500"
        >
          <HtnLogo className="h-8 w-8 text-violet-400" />
          <span>Hack the North</span>
        </Link>

        <div className="flex items-center gap-3">
          {isAuthenticated && (
            <span className="hidden items-center gap-1.5 rounded-full border border-cyan-500/20 bg-cyan-500/5 px-3 py-1 text-[10px] font-medium uppercase tracking-[0.2em] text-cyan-400 sm:inline-flex">
          ...
          {isAuthenticated ? (
            <button
              onClick={logout}
              className="inline-flex items-center gap-2 rounded-full border border-[#1e293b] bg-[#111827] px-4 py-2 text-xs font-medium uppercase tracking-[0.1em] text-slate-400 transition-all hover:border-slate-600 hover:text-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2 focus-visible:ring-offset-[#05060f]"
              aria-label="Log out"
            >
